...
//        }
        ...
        //贵州银行
        ...
        else if ("上海金币投资有限公司".equals(vo.getCustomerName())) {
            request.setAttribute("packageId", vo.getId());
            ...
            try {
                String msg5 = "**********before printShjbReceipt****";
                _logger.info(msg5);
                this.prepareForShjbPrint(request, vo, itemList, compose);
                this.generateQRCode(vo.getId());
                request.setAttribute("qrcode", this.getQrcodeUrl(vo.getId()));
                String msg6 = "**********after printShjbReceipt****";
                _logger.info(msg6);
            } catch (Exception e) {
                e.printStackTrace();
                _logger.error("****printShjbReceipt exception***", e);
            }
            ...
            try {
                String msg5 = "**********before printJlReceipt****";
                _logger.info(msg5);
                this.prepareForJlPrint(request, vo, itemList, compose);
                this.generateQRCode(vo.getId());
                request.setAttribute("qrcode", this.getQrcodeUrl(vo.getId()));
                String msg6 = "**********after printJlReceipt****";
                _logger.info(msg6);
            } catch (Exception e) {
                e.printStackTrace();
                _logger.error("****printJlReceipt exception***", e);
            }
...
//            if (OutConstant.APP_NAME_TW.equals(appName) && "上海金币投资有限公司".equals(vo.getCustomerName())){
            ...
            }  else if (vo.getCustomerName().indexOf("泰安银行") != -1) {
                return mapping.findForward("printTaReceipt");
...
//            }
    ...
    private String getCommunicationBranchNameFromOutImport(String outId){
        String branchName = "";
        if (outId.startsWith("A") || outId.startsWith("TW")){
            return branchName;
        }
        List<OutImportBean> importBeans = outImportDAO.queryEntityBeansByFK(outId, AnoConstant.FK_FIRST);
        if (!ListTools.isEmptyOrNull(importBeans))
    ...
    private void getProductCode(PackageItemBean item, Bank bank){
        String outId = item.getOutId();
        if (StringTools.isNullOrNone(outId) || outId.startsWith("A") || outId.startsWith("TW")){
...
//                item.setProductCode(this.getProductCodeFromOutImport(outId));
    ...
    private String[] getStafferNameAndPhone(String outId){
        ...
        OutInterface out = null;
        if (outId.startsWith("TW")){
            out = twOutDAO.findVO(fullId);
        } else{
            out = outDAO.findVO(fullId);
        }
...
//            totalAmount += each.getAmount();
     ...
     * @param compose
     */
    private void prepareForJlPrint(HttpServletRequest request, PackageVO vo,
                                   List<PackageItemBean> itemList, String compose)
    {
        int totalAmount = 0 ;
        ...
        for (PackageItemBean each : itemList) {
            ...
            String key = each.getProductId()+"_"+each.getItemType()+"_"+each.getLhwd();
            if (!StringTools.isNullOrNone(each.getProductCode())){
                key = each.getProductId()+"_"+each.getItemType()+"_"+each.getLhwd()+"_"+each.getProductCode();
            }
...
//				each.setDescription("");
     ...
     * @param compose
     */
    private void prepareForShjbPrint(HttpServletRequest request, PackageVO vo,
                                     List<PackageItemBean> itemList, String compose)
    {
        int totalAmount = 0 ;
...
//            }
        ...
        if (!ListTools.isEmptyOrNull(itemList1)){
            ...
            _logger.info("*****stafferName***********"+stafferName+"*******phone*************"+phone);
            String branchName = this.getBranchNameFromOutImport(firstOutId);
...
//                        String description = itemBean.getDescription() + "<br>" + each.getDescription();
     ...
     * @return
     */
    private String getRefId(OutInterface out, String outId){
        String refId = "";
