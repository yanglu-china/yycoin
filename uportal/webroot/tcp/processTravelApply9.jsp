...
<p:navigation height="22">
	<td width="550" class="navigation"><span style="cursor: pointer;"
        onclick="javascript:history.go(-1)">待我处理</span> &gt;&gt; 中收2申请处理</td>
    <td width="85"></td>
</p:navigation> <br>
...
<p:body width="100%">
	...
	<p:title>
		<td class="caption">
		 <strong>中收2申请及借款-${my:get('tcpStatus', bean.status)}</strong>
		</td>
	</p:title>
	...
	<p:subBody width="98%">
		...
		<p:table cells="2">
            ...
            <c:if test="${bean.importFlag}">
                <p:pro field="id"/>
                <c:if test="${item.ibType ==2}">
                    <p:cell title="申请类型">
                        中收2
                    </p:cell>
                </c:if>
                <c:if test="${item.ibType ==3}">
                    <p:cell title="申请类型">
                        其他费用
                    </p:cell>
                </c:if>
                        ...
                        <td>
                            <table width="100%" border="0" cellspacing='1' id="tables_ib">
                                ...
                                <c:forEach items="${bean.ibList}" var="item">
                                    <tr align="center" class="content1">
                                        <td align="center">
                                            <c:if test="${item.type ==2}">中收2
                                            </c:if>
                                            <c:if test="${item.type ==3}">其他费用
                                            </c:if>
                                        </td>
